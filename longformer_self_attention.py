...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        # apply dropout
        attn_probs = nn.functional.dropout(attn_probs,
                                           p=self.dropout,
    ...
    def _pad_and_diagonalize(chunked_hidden_states):
        ...
        chunked_hidden_states = chunked_hidden_states[:, :, :
                                                      -window_overlap]  # total_num_heads x num_chunks x window_overlap*window_overlap
        chunked_hidden_states = chunked_hidden_states.view(
    ...
    @staticmethod
    def _mask_invalid_locations(input_tensor,
                                affected_seq_len) -> torch.Tensor:
        ...
        input_tensor[:, :affected_seq_len, :, :affected_seq_len +
                     1] = torch.full_like(beginning_input,
                                          -float("inf")).where(
                                              beginning_mask.bool(),
                                              beginning_input)
        ...
        input_tensor[:, -affected_seq_len:, :,
                     -(affected_seq_len + 1):] = torch.full_like(
                         ending_input,
                         -float("inf")).where(ending_mask.bool(), ending_input)
    ...
    def _sliding_chunks_query_key_matmul(self, query: torch.Tensor,
        ...
        chunks_count = torch.div(
            seq_len, window_overlap, rounding_mode="trunc") - 1
        ...
        query = self._chunk(query, window_overlap,
                            self.config.__dict__.get("onnx_export", False))
        ...
        diagonal_attention_scores[:, 1:, :, :
                                  window_overlap] = diagonal_chunked_attention_scores[:, :, -(
                                      window_overlap +
                                      1):-1, window_overlap + 1:]

        diagonal_attention_scores[:, 0, 1:window_overlap, 1:
                                  window_overlap] = diagonal_chunked_attention_scores[:,
                                                                                      0, :
                                                                                      window_overlap
                                                                                      -
    ...
    def _sliding_chunks_matmul_attn_probs_value(self, attn_probs: torch.Tensor,
        ...
        chunked_value_size = (batch_size * num_heads, chunks_count + 1,
        ...
        return context.view(batch_size, num_heads, seq_len,
    ...
    def _concat_with_global_key_attn_probs(
        ...
        key_vectors_only_global[
            is_local_index_global_attn_nonzero] = key_vectors[
                is_index_global_attn_nonzero]
        ...
        attn_probs_from_global_key[
            ...
            is_local_index_no_global_attn_nonzero[1], :, :] = torch.finfo(
                attn_probs_from_global_key.dtype).min
        attn_probs_from_global_key = attn_probs_from_global_key.transpose(1, 3)
    ...
    def _compute_attn_output_with_global_indices(
        ...
        value_vectors_only_global[
            is_local_index_global_attn_nonzero] = value_vectors[
                is_index_global_attn_nonzero]
        ...
        attn_output_only_global = torch.matmul(
            ...
            value_vectors_only_global.transpose(1, 2).clone()).transpose(1, 2)

        # reshape attn probs
        attn_probs_without_global = attn_probs.narrow(
            -1, max_num_global_attn_indices,
            attn_probs.size(-1) - max_num_global_attn_indices).contiguous()
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        global_attn_hidden_states[
            is_local_index_global_attn_nonzero[::-1]] = hidden_states[
        ...
        global_query_vectors_only_global = (
            global_query_vectors_only_global.contiguous().view(
                max_num_global_attn_indices, batch_size * self.num_heads,
                self.head_dim).transpose(0, 1)
        )  # (batch_size * self.num_heads, max_num_global_attn_indices, head_dim)
        global_key_vectors = (global_key_vectors.contiguous().view(
            -1, batch_size * self.num_heads, self.head_dim).transpose(
                0, 1))  # batch_size * self.num_heads, seq_len, head_dim)
        global_value_vectors = (global_value_vectors.contiguous().view(
            -1, batch_size * self.num_heads, self.head_dim).transpose(
                0, 1))  # batch_size * self.num_heads, seq_len, head_dim)
        ...
        if layer_head_mask is not None:
            ...
            global_attn_probs_float = layer_head_mask.view(
                1, -1, 1, 1) * global_attn_probs_float.view(
